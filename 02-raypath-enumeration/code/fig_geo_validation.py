...
from halo_notes.draw import (PRESETS, Camera, HexPrism, RayPath, SegmentKind, annotate, finish,
                             new_figure, render_crystal, solve_raypath)

OUT = Path(__file__).resolve().parent.parent / "img" / "geo_validation.png"
WIDTH, HEIGHT, DPI = 2832, 1246, 200  # = 旧图分辨率

# 左：1-3-2 真实追迹；右：1-3-1 几何上不成立——solve_raypath 对它无解，只能画示意折线（sketch）。
# 读者要看出：从面 1 进、面 3 反射后回不到面 1
PATHS = {"1-3-2": [1, 3, 2]}
IMPOSSIBLE = [1, 3, 1]
...
def main() -> None:
    ...
    p1 = solve_raypath(c1, PATHS["1-3-2"], prefer_direction=d1,
                       prefer_point=c1.centroid(c1.face(1)) + np.array([0.5, 0.3, 0]),
    ...
    # 不可能回到顶面。先机械确认 solve_raypath 对这个序列确实无解，再画"声称的"折线示意
    # （sketch=True：不是追迹结果，图上注明"示意"）：两条都射向面 3 上同一点的入射线，
    # 第二条就是"从 3 回到 1"那段的反向
    c2 = crystal.transformed(translation=right)
    try:
        solve_raypath(c2, IMPOSSIBLE)
    except ValueError:
        pass
    else:
        raise AssertionError(f"{IMPOSSIBLE} unexpectedly has a solution; the figure's claim is wrong")
    ...
    for a_on_top in (top + np.array([-0.3, 0.35, 0]), top + np.array([0.1, 0.5, 0])):
        ...
        sketch = RayPath([a_on_top - d * geom.incident_tail, a_on_top, hit],
                         (SegmentKind.INCIDENT, SegmentKind.INTERNAL), sketch=True)
        render_crystal(ax, c2, [sketch], camera=CAMERA, preset=preset, face_numbers=True)
    annotate(ax, "光路 1-3-1：几何上不成立（示意）", c2.centroid(), (0, -1.75), camera=CAMERA,
             preset=preset, ha="center")
