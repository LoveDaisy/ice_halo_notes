...
type: directors-cut
status: published
published: "[[现代冰晕研究漫谈(3)： 平行光路的各种弧和晕]]"
url: https://zhuanlan.zhihu.com/p/485848284
---
# 现代冰晕研究漫谈(3): 平行光路的各种弧和晕

tags: [[halo]], [[Halo Notes]], [[simulation|仿真]], [[我的知乎专栏文章]],

在上一篇《[[计算机遍历-搜索-优化|现代冰晕研究漫谈(2): 计算机遍历、搜索、优化]]》中，我提到了了几个思考题，比如为什么 3-1-5-7-4 和 1-2-3-5-1 这两个看上去很不一样的光路，在柱晶形态下实际表现出来都是特里克尔弧？这两个光路之间还有什么内在的联系吗？在这篇文章中我们就尝试来探索一番。
...
![冰晶标准坐标系](img/ch3_standard_frame.png)
...
> 以下为成稿前的提纲（原 `guideline.txt`，2022），原样存档，不构成正文。

> 实际模拟可以发现, 前文规约的等价光路并不足够, 比如对于特里克尔弧, 有好几种(按前文的标准)并不等价的光路, 最后出来的弧是一样的
> 所以我们需要引入进一步的对称性
> 这里的重点是, 考虑内部反射, 可以整个融合成一个单一的矩阵, 那么他的特征值就是一个很好的等价关系划分
> 也许还要再加上头尾两个面的朝向关系, 加起来应该可以最终划分好
