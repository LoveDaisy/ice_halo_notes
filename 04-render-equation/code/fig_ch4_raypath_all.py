...
screen = CAMERA.to_world  # 按旧图观感摆姿态时用画面坐标（右/上/朝观察者）描述世界向量
...
def main() -> None:
    ...
    p2 = trace(c2, aim(c2, face_toward(c2, screen(0, -0.6, 0.8)), d2, offset=screen(0.9, 0, 0)),
               d2, ["refract", "refract"])

    # 3) 内反射：柱晶近乎直立，从左前侧面射入，在底面 1 内反射一次后从右侧面射出
    c3 = posed(HexPrism(1.0, 1.7), screen(0.12, 0.95, 0.25), 20, 4.3)
    d3 = screen(1.0, 1.0, 0.2)
    p3 = trace(c3, aim(c3, face_toward(c3, screen(-1, 0, 0.6)), d3, offset=screen(0, -0.1, 0)),
